import { github, isAuthenticated, readJson, describeCodespaceState, codespaceForwardUrl, opencodeCredentials } from '../_lib/index.js';
export default async function handler(req, res) {
  ...
  try {
    ...
    if (kind === 'running') return res.status(200).json({ status: 'running', environmentId, publicUrl, auth: opencodeCredentials(), codespaceState: codespace.state });
    ...
    return res.status(202).json({ status: 'starting', environmentId, publicUrl, auth: opencodeCredentials(), codespaceState: codespace.state, detail: 'Codespaceを起動しています…（通常1〜2分）' });
  } catch (error) {
